...
def hello_world():
    ...
    return 'python Grafana datasource, used for rendering HTML panels and timeseries data.'
...
def query_metrics():
    ...
    for target in req['targets']:
        ...
        # LATER ADDED
        add_reader(str(finder), get_sine(target, ts_range))
        # LATER ADDED
        # query_results = metric_readers[finder](target, ts_range)
        query_results = metric_readers.get(str(finder))
...
def get_sine(freq, ts_range):
    ...
    data = pd.np.sin(pd.np.arange(len(ts)) * pd.np.pi * freq * 2 / float(len(ts)))
    return pd.Series(data, index=ts).to_frame('value')
...
if __name__ == '__main__':
    ...
    app.run(host=host, port=3003, debug=True)
